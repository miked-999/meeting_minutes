...
from backend.exporter import generate_docx, generate_pdf, generate_txt, generate_srt, format_timestamp
...
def test_generate_txt_with_diarization(tmp_path):
    job = SimpleNamespace(
        id="test-job-2",
        ...
        transcript_json=json.dumps([
            ...
            {"id": 2, "start": 5.5, "end": 10.0, "speaker": "Speaker 2", "text": "Good morning Alice."}
        ]),
    ...
    assert txt_file.exists()
    content = txt_file.read_text(encoding="utf-8")
    assert "Speaker 1: [00:00 - 00:05] Good morning everyone." in content
    assert "Speaker 2: [00:05 - 00:10] Good morning Alice." in content
...
def test_generate_docx_and_pdf(tmp_path):
    job = SimpleNamespace(
        ...
        transcript_json=json.dumps([
            ...
            {"id": 2, "start": 4.5, "end": 8.0, "speaker": "Speaker 2", "text": "Agreed."}
        ]),
